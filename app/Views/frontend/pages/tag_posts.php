...
<div class="row">
    <div class="col-12">
        <h1 class="mb-4 border-bottom border-primary d-inline-block">Tag: <?= $tag ?></h1>
    </div>
            ...
            if ($pager->getPageCount() > 1): ?>
                <ul class="pagination">
                    <?php if ($pager->getCurrentPage() > 1): ?>
                        <li class="page-item">
                            <a class="page-link" href="<?= route_to('tag-posts', $tag, strval($pager->getCurrentPage() - 1)) ?>">Prev</a>
                        </li>
                    <?php endif; ?>

                    <?php for ($i = 1; $i <= $pager->getPageCount(); $i++): ?>
                        <li class="page-item <?= $i === $pager->getCurrentPage() ? 'active' : '' ?>">
                        <a class="page-link" href="<?= route_to('tag-posts-pagination', $tag, $i) ?>"><?= $i ?></a>
                        </li>
                    <?php endfor; ?>

                    <?php if ($pager->getCurrentPage() < $pager->getPageCount()): ?>
                        <li class="page-item">
                            <a class="page-link" href="<?= route_to('tag-posts-pagination', $tag, strval($pager->getCurrentPage() + 1)) ?>">Next</a>
                        </li>
                    <?php endif; ?>
                </ul>
