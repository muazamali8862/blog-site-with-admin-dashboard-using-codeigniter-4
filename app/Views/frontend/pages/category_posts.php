            ...
            if ($pager->getPageCount() > 1): ?>
                <ul class="pagination">
                    <?php if ($pager->getCurrentPage() > 1): ?>
                        <li class="page-item">
                            <a class="page-link" href="<?= route_to('category-posts-pagination', $category_slug, strval($pager->getCurrentPage() - 1)) ?>">Prev</a>
                        </li>
                    <?php endif; ?>

                    <?php for ($i = 1; $i <= $pager->getPageCount(); $i++): ?>
                        <li class="page-item <?= $i === $pager->getCurrentPage() ? 'active' : '' ?>">
                            <a class="page-link" href="<?= route_to('category-posts-pagination', $category_slug, $i) ?>"><?= $i ?></a>
                        </li>
                    <?php endfor; ?>

                    <?php if ($pager->getCurrentPage() < $pager->getPageCount()): ?>
                        <li class="page-item">
                            <a class="page-link" href="<?= route_to('category-posts-pagination', $category_slug, strval($pager->getCurrentPage() + 1)) ?>">Next</a>
                        </li>
                    <?php endif; ?>
                </ul>
